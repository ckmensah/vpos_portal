...
all_divisions = AssignedFee.where(active_status: true, del_status: false)
if all_divisions.present?
  all_divisions.each do |division|
    fee = division.entity_div_code
    find_service_code_rec = AssignedServiceCode.where(entity_div_code: fee, active_status: true, del_status: false).order(created_at: :desc).first
    if find_service_code_rec.present?
      find_service_code_rec.update(rate_status: true)
    end
  end
end
